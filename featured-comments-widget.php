...
function featured_comments_textdomain() {
	if ( function_exists('load_plugin_textdomain') ) {
		//load_plugin_textdomain('featured_comments', false, dirname(plugin_basename(__FILE__)).'/languages/');
		load_plugin_textdomain('featured_comments', false, basename( dirname( __FILE__ ) ) . '/languages' );
...
class featured_comments_widget extends WP_Widget {
    ...
    function featured_comments_widget() {
        $widget_ops = array('classname' => 'widget_featured_comments', 'description' => __('Display the comments you want to feature','featured_comments')); 
        $control_ops = array( 'width' => 550 );
        $this->WP_Widget('widget_featured_comments', __('Featured Comments','featured_comments'), $widget_ops, $control_ops);
    ...
    function form($instance) {
        $defaults = array('title' => __('Featured Comments','featured_comments'), 'count' => 5,'words' => 20); 
        $i=1; while($i<20){ $defaults['comment_'.$i] = '';  $i++; }
        $instance = wp_parse_args( (array) $instance, $defaults );
        ?>
        	<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e('Title', 'featured_comments'); ?>:</label>
        	<input type="text" name="<?php echo $this->get_field_name('title'); ?>" class="widefat" value="<?php echo esc_attr($instance['title']); ?>" /></p>

            <p><label for="<?php echo $this->get_field_id( 'count' ); ?>"><?php _e('Number Of Comments To Show', 'featured_comments'); ?>:</label>
        	<select class="widefat" id="<?php echo $this->get_field_id( 'count' ); ?>" name="<?php echo $this->get_field_name('count'); ?>"><? $i=0; while($i<20){ $i++; ?>
        	<option value="<? print $i; ?>"<? if($instance['count']==$i){ ?>selected="selected"<? } ?>><? print $i.' '.__('comments','featured_comments'); ?></option><? } ?>
   			</select></p>
   			
   			<p><label for="<?php echo $this->get_field_id( 'words' ); ?>"><?php _e('Number Of Words Per Comment', 'featured_comments'); ?>:</label>
        	<select class="widefat" id="<?php echo $this->get_field_id( 'words' ); ?>" name="<?php echo $this->get_field_name('words'); ?>"><? $i=0; while($i<200){ $i++; ?>
        	<option value="<? print $i; ?>"<? if($instance['words']==$i){ ?>selected="selected"<? } ?>><? print $i.' '.__('words','featured_comments'); ?></option><? } ?>
   			</select></p>
   				    
		    <h4><?php _e('Select Featured Comments', 'featured_comments'); ?></h4>
		    <ul>
    ...
    function widget($args, $instance) {
		...
		$i=1; while($i<$count){ 
			if(${'comment_'.$i}!=''){
				...
				echo '<li><span class="comment-meta"><span class="author">'.$comment->comment_author.'</span> '.__('in','featured_comments').' <span class="comment-title"><a href="'.get_permalink($comment->comment_post_ID).'">'.get_the_title($comment->comment_post_ID).'</a></span>:</span><blockquote>'.get_featured_comments_short_text_words($comment->comment_content,$words).'</blockquote></li>';	
			}
			$i++;
		}
		echo '</ul>';				
